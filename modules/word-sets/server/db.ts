...
export function createCategory(
  ...
  const result = db
    .prepare("INSERT INTO categories (name_zh, name_en, icon, color) VALUES (?, ?, ?, ?)")
...
export function updateCategory(
  ...
  if (updates.nameZh !== undefined)
    db.prepare("UPDATE categories SET name_zh = ? WHERE id = ?").run(updates.nameZh, id);
  if (updates.nameEn !== undefined)
    db.prepare("UPDATE categories SET name_en = ? WHERE id = ?").run(updates.nameEn, id);
  if (updates.icon !== undefined)
    db.prepare("UPDATE categories SET icon = ? WHERE id = ?").run(updates.icon, id);
  if (updates.color !== undefined)
    db.prepare("UPDATE categories SET color = ? WHERE id = ?").run(updates.color, id);
  if (updates.sortOrder !== undefined)
    db.prepare("UPDATE categories SET sort_order = ? WHERE id = ?").run(updates.sortOrder, id);
}
...
export function getCategoryWords(categoryId: number): CategoryWord[] {
  const rows = db
    .prepare("SELECT * FROM category_words WHERE category_id = ? ORDER BY sort_order, word")
...
export function reorderCategoryWords(categoryId: number, wordIds: number[]) {
  const update = db.prepare(
    "UPDATE category_words SET sort_order = ? WHERE id = ? AND category_id = ?",
  );
