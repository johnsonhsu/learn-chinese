...
import { initDatabase, getAllCategories, createCategory, updateCategory, deleteCategory, getCategoryWords, addWordToCategory, removeWordFromCategory, reorderCategoryWords } from './db.js';
...
routes.post('/admin/categories', (req, res) => {
  ...
  if (!nameZh?.trim() || !nameEn?.trim()) return res.status(400).json({ error: 'nameZh and nameEn required' });
