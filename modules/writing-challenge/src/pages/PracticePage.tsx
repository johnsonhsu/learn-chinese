...
interface Props {
  ...
  provideSession?: (userId: number) => Promise<NextSentenceResponse>;
  // What happens when a finished session is submitted (progress write-back).
  submitSession?: (userId: number, sentenceId: number, durationMs: number, charResults: CharAttemptResult[]) => Promise<SentenceResultResponse>;
...
export function PracticePage({
  ...
  const isAboveLevel = useCallback((c: string) => {
  ...
  const loadSentence = useCallback(() => {
    ...
    setCharResults([]);
    setCharResultMap({});
    setShowingChar(false); showingCharRef.current = false;
    ...
    provideSession(userId)
      .then(s => {
        ...
        window.dispatchEvent(new CustomEvent('fluency-changed', { detail: { fluency: s.fluency } }));
  ...
  const liveStatus = mustRepeat ? 'incorrect'
    : liveUsedHelp ? 'incorrect'
    : liveMistakes === 0 ? ''
    : liveFailedStrokes > 0 ? 'incorrect'
    : 'correct';
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < chars.length; i++) {
      ...
      if (rank > sentenceLevel + sentenceThreshold) {
        results.push({ char: chars[i], result: 'skip', failedStrokes: 0, hintUsed: true, durationMs: 0 });
        firstNonRed = i + 1;
      } else {
        break;
      }
  ...
  useEffect(() => { charStartTime.current = Date.now(); hintUsed.current = false; setLiveMistakes(0); setLiveUsedHelp(false); setMustRepeat(false); }, [charIndex]);
  ...
  useEffect(() => {
    if (!quizSessionMounted.current) { quizSessionMounted.current = true; return; }
  ...
  useEffect(() => {
    ...
    const lines: { label: string; value: string }[] = [
      { label: 'Lv', value: `${sentence.level} (${sentence.knownInLevel}/${sentence.totalInLevel})` },
  ...
  const finishSentence = useCallback(async (results: CharAttemptResult[]) => {
    if (!sentence) return;
    ...
    if (resp.fluency !== undefined) {
      ...
      window.dispatchEvent(new CustomEvent('fluency-changed', { detail: { fluency: resp.fluency } }));
    }
  ...
  const advanceChar = useCallback((newResults: CharAttemptResult[]) => {
    ...
    if (autoSkip) {
      ...
      while (next < chars.length && isAboveLevel(chars[next])) {
        autoResults.push({ char: chars[next], result: 'skip', failedStrokes: 0, hintUsed: true, durationMs: 0 });
        next++;
      }
  ...
  const handleCharComplete = useCallback((totalMistakes: number) => {
    // If char was showing (user pressed 👁), treat completion as a skip
    if (showingCharRef.current) {
      const attempt: CharAttemptResult = {
        char: currentChar, result: 'skip', failedStrokes: 0, hintUsed: true, durationMs: 0,
      };
      ...
      setCharResultMap(prev => prev[charIndex] !== undefined ? prev : { ...prev, [charIndex]: 'skip' });
      setShowingChar(false); showingCharRef.current = false;
    ...
    const attempt: CharAttemptResult = {
      char: currentChar, result: charResult, failedStrokes, hintUsed: usedHelp, durationMs,
    };
    const newResults = [...charResults, attempt];
    setCharResults(newResults);

    // Display map: only stores the initial attempt result per position
    setCharResultMap(prev => prev[charIndex] !== undefined ? prev : { ...prev, [charIndex]: charResult });
    ...
    return (
      <div className="sp-assessment">
        <div className="sp-center-card">
          <div className="sp-sentence-done-level">{t('practice.fluency')} {fluency} — {totalKnown} {t('practice.chars')}</div>
          {sentence?.definition && (
            <div className="sp-word-definition">{sentence.definition}</div>
          )}
          {perfectSentence && (
          ...
          {parts.length > 0 && (
            <div className="sp-sentence-done-stats">{parts.join(', ')}</div>
          )}
          {/* All chars in sentence order */}
          <div className="sp-sentence-done-chars">
            {displayResults.map((r, i) => {
              const rank = sentence?.charRanks?.[r.char] || 0;
              const rankCat = rank === 0 ? 'target' : rank > level + threshold ? 'above' : rank <= level ? 'below' : 'target';
              const isAutoSkip = r.result === 'skip' && isAboveLevel(r.char);
              const isUserSkip = r.result === 'skip' && !isAutoSkip;
              const charClass = isAutoSkip ? 'sp-result-autoskip' : isUserSkip ? 'sp-result-userskip' : `sp-result-${r.result}`;
              const mastery = sentence?.charMastery?.[r.char] ?? 0;
              const masteryColor = mastery >= 80 ? '#4caf50' : mastery >= 50 ? '#ff9800' : mastery > 0 ? '#f44336' : '#444';
              return (
                <span key={i} className={`sp-result-char ${charClass}`} onClick={() => { speak(r.char); setPracticeChar(r.char); }} style={{ cursor: 'pointer', animationDelay: `${i * 0.04}s` }}>
                  <span className={`sp-rank-corner sp-rank-corner-${rankCat}`} />
                  {r.result === 'perfect' && <span className="sp-token">⭐</span>}
                  {r.char}
                  <span className="mastery-bar"><span className="mastery-bar-fill" style={{ width: `${mastery}%`, background: masteryColor }} /></span>
                </span>
              );
            })}
          </div>
  ...
  if (error) return <div className="sp-page-empty" style={{ color: 'var(--error)' }}>{error}</div>;
  ...
  return (
    <div className="sp-assessment">
      ...
      <div className="sp-practice-top">
        {showBack && (
          <BackButton size="sm" onClick={onStop} label={t('practice.back')} />
        )}
        ...
        <div className="sp-practice-settings">
          ...
          <button className="sp-new-sentence-btn" onClick={loadSentence} title="New sentence">↻</button>
        </div>
      </div>
      ...
      <div className="sp-prompt-card">
        <div className="sp-prompt-row">
          ...
          >🔊</button>
          {sentence.definition && (
            <div className="sp-prompt-meaning">{sentence.definition}</div>
          )}
        </div>
        ...
        <div className="sp-char-scroll">
          {chars.map((c, i) => {
            ...
            const doneClass = isDone && doneResult === 'perfect' ? ' char-box-pass'
              : isDone && doneResult === 'correct' ? ' char-box-warn'
              : isDone && doneResult === 'incorrect' ? ' char-box-fail'
              : '';
            const liveClass = isActive && liveStatus === 'correct' ? ' char-box-warn'
              : isActive && liveStatus === 'incorrect' ? ' char-box-fail'
              : '';
            return (
              <span
                key={i}
        ...
        <div className="sp-top-bar">
          {showingChar ? (
            <button className="sp-skip-btn" onClick={handleSkip}>⏭</button>
          ) : (
            <button className="sp-hint-btn" onClick={() => { hintUsed.current = true; setLiveUsedHelp(true); canvasRef.current?.showHint(); }}>💡</button>
          )}
          <button
