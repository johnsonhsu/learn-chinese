...
import {
  getCharacterStats as getPlatformCharacterStats,
  computeTodayScore as sharedComputeTodayScore,
  computeRetention as sharedComputeRetention,
  masteryConfigFromSettings,
  type MasteryConfig as SharedMasteryConfig,
} from "@shared/character-stats";
...
import {
  isCharKnown as isCharKnownShared,
  getTargetChars as getTargetCharsShared,
} from "@shared/character-stats/char-knowledge";
...
export function getRankedChars(): RankedChar[] {
  ...
  const pdb = new Database(join(__dirname, "..", "..", "..", "platform", "platform.db"), {
    readonly: true,
  });
  try {
...
export function isCharKnown(stat: {
...
export function computeUserLevel(userId: number): {
  level: number;
  knownInLevel: number;
  totalInLevel: number;
  fluency: number;
...
export function getTargetChars(userId: number): {
  chars: string[];
  level: number;
  knownInLevel: number;
  totalInLevel: number;
...
export function generateNextSentence(userId: number): {
  ...
  const bankSentences = getAllBankSentences().map((b) => ({
    sentence: b.sentence,
    english: b.english,
  }));
  ...
  const {
    ...
    fluency,
    totalKnown,
  } = getTargetCharsShared(ranked, stats, settings);
...
export function computeCoverage(userId: number): {
  ...
  const tocflWords = withContentDb(
    (cdb) =>
  ...
  const baseChars = JSON.parse(
    readFileSync(join(__dirname, "..", "src", "data", "base-chars.json"), "utf-8"),
  );
  let totalMovieW = 0,
    knownMovieW = 0,
    totalBookW = 0,
