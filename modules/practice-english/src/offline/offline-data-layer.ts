/**
 * Offline data layer for learnenglish.
 * Loads content.db (curated bank sentences) and tracks English word mastery in
 * IndexedDB. The sentence bank is platform-owned content now (was the
 * writing-challenge module DB).
 */
...
import { listProfiles, putProfile, createProfile, deleteProfile,
  getProfileWordStats, putProfileWordStats, getPref, setPref, type WordStatRecord, type Profile } from './user-store.js';
import { selectNextSentence, type Sentence, type SpellQuestion } from '../cloze.js';
...
export class OfflineDataLayer {
  ...
  createProfile(name: string): Promise<Profile> { return createProfile(name); }
  async deleteProfile(id: number): Promise<void> { await deleteProfile(id); }
  ...
  async updateSettingsPrefs(patch: Partial<{ language: string; displayName: string }>): Promise<void> {
    if (patch.language !== undefined) await setPref('language', patch.language);
    if (patch.displayName !== undefined) await this.renameProfile(this.userId, patch.displayName);
  }
  ...
  getNextSentence(): SpellQuestion | null {
    const mastered = this.getMasteredWords();
    return selectNextSentence(this.sentences, mastered, this.recentSentenceIds);
  }

  async submitResult(sentenceId: number, word: string, correct: boolean): Promise<void> {
