...
/**
 * Module-mode provider (issue #69). The PLATFORM owns profiles, language and the
 * shell — here we spin up reading-english's own data layer for the given platform
 * `userId`, read the shared bank (/data/content.db) and track this profile's
 * per-word READING mastery in a store that is disjoint from practice-english's
 * spelling store.
 */
export function OfflineProvider({ userId, children }: { userId: number; children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    dl.initialize()
      ...
      .catch((err) => console.error('reading-english init failed:', err));
