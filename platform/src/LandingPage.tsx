...
const T = {
  en: {
    ...
    readLede:
      "Set how many of the most common characters you know, then watch real Taiwan text come into focus.",
    ...
    readCustomPlaceholder:
      "Paste any Traditional Chinese — a menu, a text, an article — and see how much you could read right now.",
    ...
    pitchBody:
      "One short session — the kind you finish on the MRT. Show up daily and the characters stick, because each one comes back for review right when you’re about to forget it.",

    // ── Milestones: concrete reading outcomes at each character count ──
    mileEyebrow: "Three milestones",
    m1n: "500",
    m1l: "Everyday life",
    m1when: "first milestone",
    ...
    m3n: "1,500",
    m3l: "News & beyond",
    m3when: "going further",
    m3d: "Read everyday text comfortably and start making real headway on news, notices, and longer articles.",
    mileFoot:
      "You don’t need 5,000 characters to live your life in 中文. You need the right few hundred, in the order that pays off fastest. How fast you get there is up to you.",

    // ── Proof: the app turns progress into a number ──
    proofEyebrow: "Progress you can see",
    p1n: "Known",
    p1d: "Every character you’ve learned, counted — watch it climb past 500, then 1,000.",
    p2n: "Retention",
    p2d: "Spaced review tracks what’s actually stuck, so you know it’s in for good — not just seen once.",
    p3n: "Level",
    p3d: "One number for how far you’ve come — it climbs as you learn, based on the characters you can actually read right now.",

    // ── Why it works ──
    why: "How it works",
    s1t: "Frequency order",
    s1: "You learn the most-used characters first, so a small set buys a huge amount of reading — fast.",
    s2t: "Spaced repetition",
    s2: "Each character comes back right before you’d forget it. The better you know it, the less it returns.",
    s3t: "Write to remember",
    s3: "Trace every character stroke by stroke on a real canvas, checked as you go. Writing it is what makes it stick.",

    // ── Reassurance ──
    reassureEyebrow: "No catch",
    f1: "Free, forever",
    f2: "Works fully offline",
    f3: "No account, no sign-up",
    f4: "No ads, ever",
    f5: "Your data stays on your device",
    f6: "Real Taiwan Traditional + Zhuyin",
    ...
    installIntroIos:
      "It runs like a real app and keeps every character you learn on your device. On your iPhone or iPad, in Safari:",
    i1: "Tap the Share button",
    i1b: "in the toolbar.",
    ...
    installIntroGeneric:
      "It installs like a real app and keeps every character you learn on your device:",
  ...
  "zh-TW": {
    ...
    readCustomTab: "自訂",
    readCustomPlaceholder:
      "在這裡貼上任何繁體中文——菜單、訊息、文章都可以——看看以你現在的程度能讀懂多少。",
    ...
    pitchBody:
      "一次短短的練習——搭捷運的時間就能做完。每天持續，字自然就記住了，因為每個字都會在你快要忘記的那一刻，剛好再出現一次。",
    ...
    mileFoot:
      "要用中文過生活，你不必學會五千個字。你需要的是對的那幾百個字——而且照著最划算的順序去學。至於要花多久，就看你自己了。",

    proofEyebrow: "進度看得見",
    p1n: "已學會",
    p1d: "你學會的每個字都會算進來。看著它超過 500，再超過 1,000。",
    p2n: "記憶率",
    p2d: "間隔複習會告訴你哪些字是真的記住了——不只是「看過一次」。",
    p3n: "等級",
    p3d: "把你的進度濃縮成一個分數——它會隨著你現在真正讀得懂的字，一路往上爬。",

    why: "為什麼有效",
    s1t: "頻率排序",
    s1: "先學最常用的字，所以一小組字就能換來大量的閱讀能力——而且很快。",
    s2t: "間隔複習",
    s2: "每個字都會在你快忘記前再出現。記得越熟，複習就越少；沒有一次練習被浪費。",
    s3t: "用寫的記住",
    s3: "在真實的畫布上一筆一畫描寫每個字，邊寫邊檢查。動手寫過，記得最牢。",
    ...
    installBarIos: "加入主畫面",
    installIntroIos:
      "它會像真正的 App 一樣運作，進度也都存在你的裝置上。在 iPhone 或 iPad 的 Safari 裡：",
    i1: "點一下工具列的「分享」按鈕",
    i1b: "。",
    i2: "往下滑，點一下「加入主畫面」。",
    i3: "從主畫面開啟「學中文」，就完成了。",
...
function IosShareIcon() {
  return (
    <svg className="lp-share-ico" viewBox="0 0 24 24" width="18" height="18" aria-hidden="true">
      ...
      />
      <path
        d="M6.5 11H5.5a1.5 1.5 0 00-1.5 1.5V19A1.5 1.5 0 005.5 20.5h13A1.5 1.5 0 0020 19v-6.5A1.5 1.5 0 0018.5 11h-1"
        fill="none"
        stroke="currentColor"
        strokeWidth="2.2"
        strokeLinecap="round"
...
function PracticeCell({ glyph }: { glyph: string }) {
  return (
    <div className="lp-cell" aria-hidden="true">
      ...
      <span className="lp-cell-glyph" lang="zh-TW">
        {glyph}
      </span>
    </div>
  );
...
function OpenBook({ t, lang }: { t: (typeof T)[Lang]; lang: Lang }) {
  ...
  function onTabKeyDown(e: ReactKeyboardEvent<HTMLDivElement>) {
    ...
    if (e.key === "ArrowRight" || e.key === "ArrowDown") next = (cur + 1) % tabKeys.length;
      ...
      next = (cur - 1 + tabKeys.length) % tabKeys.length;
    ...
    tablistRef.current?.querySelector<HTMLButtonElement>(`#lp-read-tab-${key}`)?.focus();
  }
  ...
  return (
    <section
      id="try"
      className="lp-section lp-book-section lp-reveal"
      aria-labelledby="lp-read-head"
    >
      <p className="lp-eyebrow">{t.readEyebrow}</p>
      <h2 id="lp-read-head" className="lp-read-head">
        {t.readHead}
      </h2>
        ...
        <div className="lp-note-body">
          ...
          <div className="lp-read-control">
            ...
            >
              ...
              <span className="lp-read-tick-label" style={{ left: "0%" }}>
                0
              </span>
              <span className="lp-read-tick-label" style={{ left: "100%" }}>
                1,500
              </span>
                  ...
                  return scored.chars.map((ch, i) => {
                    ...
                    return (
                      <span
                        ...
                        style={
                          { "--lp-i": h, transitionDelay: `${(i % 8) * 16}ms` } as CSSProperties
                        }
                        aria-hidden="true"
                      >
                        {ch.c}
...
function scrollToSectionId(id: string) {
  ...
  const barH = root ? parseInt(getComputedStyle(root).getPropertyValue("--lp-bar-h")) || 56 : 56;
...
function NavDropdown({
  ...
  return (
    ...
    >
      ...
      <div id="lp-navmenu" className={`lp-navmenu${open ? " is-open" : ""}`} aria-label={title}>
        ...
        <div className="lp-navmenu-cta" onClick={close}>
          {cta}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="lp" lang={lang} ref={lpRef}>
      ...
      <header className="lp-bar">
        <div className="lp-bar-inner">
          ...
          <NavDropdown
            ...
            cta={<Cta block />}
          />
          <span className="lp-mark" ref={markRef}>
            {t.wordmark}
          </span>
      ...
      {/* Brand splash: the large 學中文 that SHRINKS INTO the bar's wordmark as you
          ...
          the hero down. */}
      <span className="lp-fly" ref={flyRef} aria-hidden="true">
        {t.wordmark}
      </span>
      <div className="lp-splash" aria-hidden="true">
        <span className="lp-splash-word" ref={splashRef}>
          {t.wordmark}
        </span>
      </div>
      <section id="top" className="lp-hero">
        <h1 className="lp-title">
          ...
          {"heroBig2" in t && t.heroBig2 ? (
            <span className="lp-title-big">{t.heroBig2}</span>
          ) : null}
        </h1>
        <PracticeCells />
      </section>
          ...
          {installState === "ios" ? (
            <>
              ...
              <ol className="lp-install-steps">
                <li>
                  <span className="lp-dot">1</span>
                  <span>
                    {t.i1} <IosShareIcon /> {t.i1b}
                  </span>
                </li>
                <li>
                  <span className="lp-dot">2</span>
                  <span>{t.i2}</span>
                </li>
                <li>
                  <span className="lp-dot">3</span>
                  <span>{t.i3}</span>
                </li>
              </ol>
            </>
