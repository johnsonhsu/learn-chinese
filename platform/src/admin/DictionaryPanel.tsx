...
function CharPreview({ character }: { character: string }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const charDataLoader = (char: string, onLoad: (data: any) => void) => {  
        fetch(`/stroke-data/${encodeURIComponent(char)}.json`)
          ...
          .then(onLoad)
          .catch(() => fetch(`https://cdn.jsdelivr.net/npm/hanzi-writer-data@2.0.1/${encodeURIComponent(char)}.json`).then(r => r.json()).then(onLoad).catch(() => {}));
      };
      ...
      const animWriter = HanziWriter.create(animRef.current, character, {
        ...
        strokeColor: '#4a90d9', outlineColor: borderColor,
    ...
    } catch { /* char not in hanzi-writer */ }
    ...
    return () => {
      ...
      try { writerRef.current?.pauseAnimation(); } catch { /* ignore */ }
...
export function DictionaryPanel() {
  ...
  const loadChars = useCallback((dictId: number, q: string, off: number, tocfl: boolean, sort: string = 'freq') => {
    if (sort === 'blended') {
      ...
      Promise.all([
        ...
        api<CharEntry[]>(`/dictionaries/${dictId}/chars?q=${encodeURIComponent(q)}&offset=0&limit=9999${tocfl ? '&tocfl=1' : ''}`),
      ]).then(([ranking, allChars]) => {
        const rankMap = new Map(ranking.map(r => [r.char, r.rank]));
        const sorted = allChars.sort((a, b) => (rankMap.get(a.character) || 99999) - (rankMap.get(b.character) || 99999));
        setChars(sorted.slice(off, off + 50));
      });
    } else {
      api<CharEntry[]>(`/dictionaries/${dictId}/chars?q=${encodeURIComponent(q)}&offset=${off}&limit=50${tocfl ? '&tocfl=1' : ''}`)
        .then(setChars);
    }
  }, []);

  const loadWords = useCallback((dictId: number, q: string, off: number, tocfl: boolean) => {
    api<WordEntry[]>(`/dictionaries/${dictId}/words?q=${encodeURIComponent(q)}&offset=${off}&limit=50${tocfl ? '&tocfl=1' : ''}`)
      .then(setWords);
  }, []);
    ...
    return (
      <div className="dict-panel">
        <div className="dict-header">
          <button className="back-btn" onClick={() => setCharDetail(null)}>← Back</button>
  ...
  return (
    <div className="dict-panel">
      <div className="dict-header">
        <button className="back-btn" onClick={() => setSelectedDict(null)}>← Dictionaries</button>
        <span>{selectedDict.name}</span>
      </div>

      <div className="dict-controls">
        <div className="dict-tabs">
          <button className={`dict-tab${view === 'chars' ? ' active' : ''}`} onClick={() => handleViewChange('chars')}>
            Chars
          </button>
          <button className={`dict-tab${view === 'words' ? ' active' : ''}`} onClick={() => handleViewChange('words')}>
            Words
          </button>
          ...
          {view === 'chars' && (
            <button className={`dict-tab${charSort === 'blended' ? ' active' : ''}`} onClick={() => {
        ...
        <div className="dict-char-grid">
          {chars.map(c => (
            <button key={c.id} className="dict-char-cell" onClick={() => handleCharClick(c.id)}>
              ...
              <span className="dict-char-strokes">{c.strokeCount > 0 ? `${c.strokeCount}画` : ''}</span>
              {c.tocfl_level && <span className="dict-char-level">{c.tocfl_level}</span>}
            </button>
          ))}
        </div>
      ) : (
      ...
      <div className="dict-pager">
        <button disabled={offset === 0} onClick={() => handlePage(-1)}>← Prev</button>
        <span>{offset + 1}–{offset + (view === 'chars' ? chars.length : words.length)}</span>
