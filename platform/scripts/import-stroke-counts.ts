...
const chars = db
  .prepare("SELECT id, character FROM dict_chars WHERE stroke_count = 0 OR stroke_count IS NULL")
...
async function main() {
  ...
  // Stats
  const withStrokes = (
    db.prepare("SELECT COUNT(*) as n FROM dict_chars WHERE stroke_count > 0").get() as { n: number }
  ).n;
  const without = (
    db.prepare("SELECT COUNT(*) as n FROM dict_chars WHERE stroke_count <= 0").get() as {
      n: number;
    }
  ).n;
