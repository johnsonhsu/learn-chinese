...
interface R2Bucket {
  put(_key: string, _value: ArrayBuffer | string): Promise<unknown>;
}
...
export async function onRequestPost(context: { request: Request; env: Env }) {
  ...
  try {
    ...
    return Response.json(
      { error: "store failed", detail: String(e).slice(0, 200) },
      { status: 500 },
    );
  }
}
...
export async function onRequestGet(context: { request: Request; env: Env }) {
  ...
  const items = status
    ? (
        await env.FEEDBACK_DB.prepare(`${base} WHERE status = ?1 ORDER BY id DESC LIMIT ?2`)
          .bind(status, limit)
          .all()
      ).results
    : (await env.FEEDBACK_DB.prepare(`${base} ORDER BY id DESC LIMIT ?1`).bind(limit).all())
        .results;
