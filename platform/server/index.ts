...
app.post("/api/character-stats/record", (req, res) => {
  ...
  if (!userId || !char || !result)
    return res.status(400).json({ error: "userId, char, result required" });
  recordCharacterAttempt(userId, char, {
    result,
    failedStrokes: failedStrokes || 0,
...
app.get("/api/dictionaries", (_req, res) => {
  ...
  const result = dicts.map((d) => {
    const charCount = (
      db.prepare("SELECT COUNT(*) as n FROM dict_chars WHERE dictionary_id = ?").get(d.id) as {
        n: number;
      }
    ).n;
    const wordCount = (
      db.prepare("SELECT COUNT(*) as n FROM dict_words WHERE dictionary_id = ?").get(d.id) as {
        n: number;
      }
    ).n;
    const linkCount = (
      db
        .prepare(
          "SELECT COUNT(*) as n FROM dict_char_words cw JOIN dict_chars c ON c.id = cw.char_id WHERE c.dictionary_id = ?",
        )
    ...
    const withStrokes = (
      db
        .prepare(
          "SELECT COUNT(*) as n FROM dict_chars WHERE dictionary_id = ? AND stroke_count > 0",
        )
        .get(d.id) as { n: number }
    ).n;
...
app.get("/api/dictionaries/:id/char/:charId", (req, res) => {
  ...
  const char = db.prepare("SELECT * FROM dict_chars WHERE id = ?").get(charId) as
    { id: number; character: string; stroke_count: number } | undefined;
  if (!char) return res.status(404).json({ error: "not found" });

  const meta = db
    .prepare("SELECT key, value FROM dict_char_metadata WHERE char_id = ?")
    .all(charId) as { key: string; value: string }[];
  const words = db
    .prepare(
      `
