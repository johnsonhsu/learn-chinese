...
import {
  addBankSentences, getBankSentenceCount, getAllBankSentences, deleteAllBankSentences,
  searchBankSentences, updateBankSentence, deleteBankSentence, deleteBankSentences,
  restoreBankFromBaked, getCharTocflLevels,
...
const MODULE_SETTINGS_DB_PATH = join(repoRoot, 'modules', 'writing-challenge', 'writing-challenge.db');
...
function getRankSettings(): Record<string, string> {
  ...
  try {
    ...
    const rows = mdb.prepare('SELECT key, value FROM module_settings').all() as { key: string; value: string }[];
    for (const r of rows) out[r.key] = r.value;
    mdb.close();
  } catch { /* module DB absent — ranker falls back to its own defaults */ }
  return out;
}
...
contentAdminRoutes.get('/admin/bank-sentences', (req, res) => {
  ...
  const sentences = searchBankSentences(q, limit).map((s) => ({ ...s, difficulty: bankDifficulty(s.sentence, rankMap) }));
...
async function generateGemini(prompt: string, bodyKey: string): Promise<string> {
  ...
  if (!apiKey) throw httpError(503, 'No Gemini key — set GEMINI_API_KEY in your dev .env or save a key in Settings');
...
async function generateCloudflare(prompt: string): Promise<string> {
  ...
  if (!accountId || !token) {
    throw httpError(503, 'No Cloudflare Workers AI config — set CF_ACCOUNT_ID and CF_AI_TOKEN in your dev .env');
  }
...
contentAdminRoutes.post('/admin/ai-generate', async (req, res) => {
  ...
  try {
    const text = provider === 'cloudflare'
      ...
      : await generateGemini(prompt, typeof req.body?.apiKey === 'string' ? req.body.apiKey.trim() : '');
    res.json({ text });
  } catch (e) {
