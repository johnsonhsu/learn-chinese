...
- **自己的建置入口／網址。** 第二個 Vite HTML 入口（`platform/feedback-admin.html` +
  ...
  **同一個 `dist`**，並由**同一個 `pages deploy dist`** 一起部署。它**只能透過直接網址**到達——
  `/feedback-admin.html`，或 `platform/public/_redirects` 裡乾淨的 `/feedback-admin` 改寫（排在 SPA
  萬用規則之前）。它是 `noindex`／`no-store`（`platform/public/_headers`）。
