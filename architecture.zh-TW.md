...
- 新增的 **`quizSession`** prop 讓父層能要求對 _同一個_ 字形重新 quiz（失敗後的必須重練，或連續重複的
...
2. **種子 + 版本檢查。** `ensureDemoSeed()`（於 `OfflineProvider` 在 `initialize()` 後呼叫）以字頻排名
   ...
   `shouldShowLanding` 或加 `_redirects`——刻意延後；`?app&demo` 已涵蓋示範入口，暫不需要。）
...
**裝置閘門的桌面測試跳轉參數 (`?nodevicegate`, #76).** 為讓桌面 reviewer 也能從
PR preview 直接進入 demo，新增 query param `?nodevicegate`：一旦出現，
`isDemoDeviceAllowed()` 直接回傳 `true`，使 `isDemoDeviceGated()`=`false`、
`<DemoGate>` 不會被渲染。CI preview comment 也會補上
`${prBase}/?app&demo&nodevicegate`，方便直接點擊。這**不是安全性控制**：它只讓
desktop visitor 進入一個已經隔離的 demo jar，在 prod 環境下暴露也無害。
...
一套由註冊表（registry）驅動的主題系統，在預設外觀之上疊加一層可選的替代皮膚。今天出貨 **14 款主題**
（`theme/themes.ts`），依選擇器分組（#129）：**default**（Indigo——預設*選擇*——與 Paper，即不設 token 的
`:root` 基準）、**dark**（Midnight、Outer Space）、**soft／節慶**（Sakura、Matcha、Jungle、Christmas）、
**retro**（90s、80s Motiv）、**Disney**（Boyish、Girlish），以及 **foil**——兩款付費皮膚 **Gold**（暖金箔）
與 **Silver**（冷鉑金）。新增一款只需在註冊表中加一筆。

- **契約 + 目錄** 是 `theme/themes.ts`：一個 `THEME_TOKENS` 允許清單（一個主題可設定的具名 CSS custom
  properties——背景、foil 家族、卡片臉／框／字形、按鈕家族、文字家族／縮放／字重，以及模組選擇的
  `arrangement`）加上 `THEMES` 註冊表（`id`、`name`、`premium`、`arrangement`、`group`）。**Paper**（註冊表 id
  `default`）不設定任何 token——`:root` 的編輯設計值即為其外觀，因此該基準外觀與導入主題前位元組完全相同；
  **Indigo** 才是預設*選擇*。
...
- **付費閘門。** Gold/Silver 是**唯二**的 `premium: true` 主題，各自以**自己的代碼獨立**、**僅限裝置層級**
  ...
  **`9901` → Gold**（每個主題項帶有 `unlockFeature` 鍵——Silver ← `theme-silver`、Gold ← `theme-gold`）。
  ...
  「代碼無效」，不暗示其為真——見 §5.6）。_向後相容：_ 已存有舊版
  ...
  _覆寫_（`theme-store.ts` 的 `isThemeAvailable()` 為逐主題判斷；`isDevicePremiumUnlocked()` 為粗略的
...
App 以直式為主，但也支援橫向。所有橫向規則都包在 `@media (orientation: landscape)`
內，因此直式維持完全不變（pixel-identical）。橫向時置中欄位變寬（首頁／使用者／設定／
引導），書寫板則縮放以配合較矮的橫向高度——畫布本來就會依容器 CSS 縮放（與窄螢幕直式
同一路徑），觸控座標對應不受影響。閱讀模組與字／詞格已是流式版面（`--content-width` +
`auto-fill`），會自動運用多出的寬度。

**方向政策（`useOrientationLock.ts`）**：以「鎖定直式」設定為唯一依據。關閉（預設）時
App 跟隨裝置方向（直式或新的橫向版面）；開啟時使用者明確選擇直式，因 iOS/iPadOS 網頁
無法強制系統方向，改以全螢幕「請轉回直式」覆蓋層（`.rotate-overlay`，由 `html.lock-portrait`
控制）阻擋橫向，其關閉鈕透過真正的設定儲存（`window.__setPortraitLock` → `updateSettings`，
示範模式隔離）關掉鎖定，而非 `localStorage`（舊的無效寫入 bug，議題 #112）。覆蓋層刻意
保留，作為選擇直式使用者的合理退路。
...
- **已移除：** `9999`（舊版總括付費）與 `8888` 不再兌換。_向後相容：_ 已存有 `premium` 的裝置保有兩款金屬皮膚；
...
- **每 IP 限流。** POST handler 以 `cf-connecting-ip` 為單位限流，使用一張小的 `rate_hits` D1 表
  ...
  擋住正常的送出。
- **管理閘門。** `GET`／`PATCH`／截圖路由由共用 secret（`FEEDBACK_ADMIN_SECRET`）把關，**只接受
  `x-feedback-admin-secret` header**——絕不接受 `?secret=` URL 參數（那會把 secret 洩漏到 CF／存取
  日誌、瀏覽器歷史與 `Referer`；安全稽核 **M2／#55**）。triage 介面會以該 header 抓取截圖位元組，並渲染成
  object-URL（unmount 時撤銷），讓 secret 不會出現在 `<img src>` 中。以近乎定時的方式比對。若 secret
...
### 獨立 triage 介面（`/feedback-admin`，議題 #59）

回饋 triage 是一個**獨立、未連結的介面**，刻意與主學習 app **解耦**——它是擁有者的私人分流主控台，並非
學習者 PWA 的一部分。它是**唯一**的回饋管理介面（**取代**了先前 Admin 主控台裡的 `feedback` 分頁，該分頁
已移除）。

- **自己的建置入口／網址。** 第二個 Vite HTML 入口（`platform/feedback-admin.html` +
  `platform/src/feedback-admin.tsx`，在 `platform/vite.config.ts` 以 `rollupOptions.input` 接上）會產出到
  **同一個 `dist`**，並由**同一個 `pages deploy dist`** 一起部署。它**只能透過直接網址**在乾淨路徑
  **`/feedback-admin`** 到達——Cloudflare Pages 內建的 clean-URL 處理會把靜態檔 `feedback-admin.html`
  以這個無副檔名路徑提供（並把明確的 `/feedback-admin.html` 以 308 導向 `/feedback-admin`），就像它把
  `index.html` 提供在 `/` 一樣，因此**不需要 `_redirects` 規則**。⚠️ 不可再加
  `/feedback-admin → /feedback-admin.html 200` 改寫：它會和 clean-URL 層互撞形成無限 308 迴圈
  （`ERR_TOO_MANY_REDIRECTS`）——這正是 PR #67 修掉的 bug；`platform/public/_redirects` 內留有警告註解。
  它是 `noindex`／`no-store`（`platform/public/_headers`）。
- **雙向都沒有 UI 導覽。** 該入口**不 import** 任何 `App` 的東西，只掛載 triage 元件
  （`platform/src/feedback-admin/FeedbackTriage.tsx`），也不連回 app；app 也不連到它。已驗證：建置出的
  `index.html` 對 feedback-admin 入口零引用。
- **不是 PWA 頁。** 沒有 manifest、沒有 service worker 註冊、沒有離線殼、也沒有 app 殼／主題 CSS（它用一份
  很小、自足的 `platform/src/feedback-admin.css`）。它自己的 HTML + 入口 JS／CSS 都**排除在 app 的 SW
  precache 之外**（`vite.config.ts` 的 `workbox.globIgnores`；入口有固定的 `feedback-admin-entry.js` 檔名讓
  glob 能鎖定），也不會被拉進 app bundle。
- **正式環境讀取路徑、重用端點。** 擁有者輸入一次管理 secret；**「解鎖」會先向伺服器探測**（以 header 打
  `GET /api/feedback?limit=1`），只有回 `200` 才把 secret 存進 `localStorage`——回 `403` 則在解鎖畫面顯示清楚
  原因，而不是先默默存起來、再在第一個列表／狀態／截圖請求時把使用者踢回「上鎖」。因為無論 secret 是*錯的*還是
  _這個部署根本沒設定_，端點都回**無法區分的 `403`**，解鎖錯誤訊息會同時點出兩種原因——後者是 **PR PREVIEW
  部署的預期狀態**，因為 `FEEDBACK_ADMIN_SECRET` 只綁在 Pages 專案的**正式（production）**環境（見下方佈建
  runbook）。解鎖*之後*才出現的 `403`（secret 被輪替／該部署沒設）會用同一段訊息重新上鎖。secret 在執行期輸入、
  **絕不打包進 bundle**。介面接著讀取**既有的**、受管理閘門保護且 siloed 的端點——`GET /api/feedback`
  （列表 + `?status=` 篩選 + 計數）、`PATCH /api/feedback/:id`（設定狀態）、`GET /api/feedback/:id/screenshot`
  （截圖位元組）——secret 只以 **`x-feedback-admin-secret` header** 傳送。它以最新優先顯示回饋，提供**依狀態
  的篩選 chips**（與計數）、行內**狀態變更**、每列一行的**不含 PII 上下文**，以及延遲載入的**截圖縮圖**
  （以 header 抓取、渲染成 object-URL；點擊放大）。從它無法觸及任何 app／使用者／內容相關資料——siloing 得以
  保留，因為那些 Function 只綁定回饋的 D1／R2 + secret。
...
- `feedback-routes.ts` — 相同的四條路由（`POST` 公開；`GET`／`PATCH`／`:id/screenshot` 由開發 `.env` 的
  `FEEDBACK_ADMIN_SECRET` 把關，**僅接受 header**——不接受 `?secret=`，與正式環境一致）、相同的驗證，以及
  一個記憶體內的每 IP 限流器。開發時同一個獨立 `/feedback-admin` 入口由 Vite（app 的 `:3000` 伺服器）提供，
  並打這些路由。
...
#    注意：`pages secret put` 只會寫入「正式（production）」環境。
...
> ⚠️ **Preview 與 Production。** Cloudflare Pages 的 **Production** 與 **Preview** 兩個環境各自有**獨立**的
> secrets／bindings。`wrangler pages secret put`（步驟 4）只寫入 **Production** secret，因此在 **PR preview**
> 部署上 `FEEDBACK_ADMIN_SECRET` 是**未設定**的，無論你輸入什麼 secret，`/api/feedback*` 都會回 `403`（fail-closed）
> ——`/feedback-admin` 主控台在解鎖時會如實回報。這是預期且無害的（在 **prod** 上 secret 有綁定，主控台可正常運作）。
> 若想在 preview 上也能操作 triage，請參見下方的 **Preview 環境佈建**。

### Preview 環境佈建（issue #78）

一次性 owner action：佈建 **專屬的 preview feedback stack** 並將它**只**
綁定到 **Preview** 環境。Cloudflare Pages 的所有非 production branch 共用
同一份 Preview 設定，因此每個 PR preview 都會自動使用這些資源——同時保持
**與 production 隔離**：

```bash
# 1. 建立專屬的 preview D1 資料庫。
npx wrangler d1 create feedback-preview

# 2. 對 preview DB 套用與 prod 一致的 schema。
npx wrangler d1 execute feedback-preview --remote \
  --file=platform/functions/migrations/0001_init.sql

# 3. 專屬的 preview R2 bucket 存放截圖。
npx wrangler r2 bucket create learning-chinese-feedback-preview

# 4. 設定一組**與 prod 不同**的 preview admin secret。
#    注意：`pages secret put` 只寫入 Production。
#    請透過 Pages dashboard 的 Preview environment 設定，或對應的
#    preview-aware wrangler 機制建立 secret，並確認它落在
#    Preview，而非 Production。
```

接著，在 Pages 專案中（**Settings → Functions → Bindings → Preview
environment**），只加入：

- `FEEDBACK_DB` → `feedback-preview`
- `FEEDBACK_R2` → `learning-chinese-feedback-preview`

**Prod 必須維持 untouched。** Preview 與 production **不得**共用 D1、R2
或 `FEEDBACK_ADMIN_SECRET`。把 preview 資源只綁在 Preview 環境，可確保
preview 測試資料不會掉進 production，也不會因為 preview secret 外洩而
影響 production。

**綁定生效後需要重新部署 preview。** 綁定在 deploy 時才 attached，因此請
push 一個新的 PR branch commit 或重新打開 PR。驗證方式：

- 在 preview URL 送出 feedback → 資料列寫入 `feedback-preview`
- 在 preview URL 對 `/feedback-admin` 使用 preview secret → `200`
- preview 測試後，production `feedback` 的列數維持不變
...
_選擇 profile 之前_，因為這裡的一切都屬於裝置或帳號層級，而非單一 profile。返回會回到選擇器。各區塊由上到下：
...
| _（受閘門的按鈕）_                  | **UI 元件** → Styleguide（僅開發）。**管理（Admin）** → Admin 主控台（§8.4）。                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Styleguide **只**在 `import.meta.env.DEV` 下渲染；**Admin** 在 `import.meta.env.DEV` 下、**或** `admin` 功能經代碼 `8000` 再 `8001` 解鎖後渲染。 |
...
> _按鈕_ 可在正式環境開啟（開發版建置，或 `8000`+`8001` 的 admin 解鎖），但它背後的路由——`/api/admin/*`、
