...
**Demo 範圍的 localStorage 與 always-fresh 重置。** 可設定的 localStorage 通道（unlocks、主題、語音、
Gemini 金鑰、字帖文字、auto-skip 開關）不在 demo 的 IndexedDB jar 內，因此其基底名稱透過 `demoKey()`
（`offline/demo-key.ts`）在 demo 模式下導向隔離的 `<base>-demo` 變體；`resetDemoKeys()`（由
`ensureDemoSeed` 呼叫）會清除這些 `-demo` 鍵，使 demo 永遠保持全新。**唯一例外（issue #63）：** demo 的
**unlocks** 鍵（`lc-unlocks-demo`）由 `resetDemoKeys` **保留**——訪客本次 session 兌換的代碼是刻意的使用者
動作，而在 demo 模式下 always-fresh 重置**也**會在導覽觸發的 SW 自動重載時觸發（`useAppUpdate.ts` 的
`onNeedRefresh`），所以清掉 unlocks 會在 session 中途把使用者踢出 admin／重新隱藏 premium，並把下一個串接
代碼判為「代碼無效」。此豁免就是 `demo-key.ts` 中的單一 `PRESERVED_DEMO_KEYS` 集合，與 `utils/unlocks.ts`
共用 `UNLOCKS_BASE_KEY` 以免名稱漂移；隔離不變（仍是 `-demo` 變體，永不碰真實的 `lc-unlocks`），且全新的
demo 瀏覽器仍以「未解鎖任何項目」開始。
