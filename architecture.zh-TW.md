...
- **付費閘門。** Gold/Silver 是**唯二**的 `premium: true` 主題，各自以**自己的代碼獨立**、**僅限裝置層級**
  ...
  **`9901` → Gold**（每個主題項帶有 `unlockFeature` 鍵——Silver ← `theme-silver`、Gold ← `theme-gold`）。
  `9000` 本身**不顯示任何主題**；在 `9000` 之前輸入 `99xx` 會被**拒絕**（前置缺失）。*向後相容：* 已存有舊版
...
`{ status: 'prerequisite-missing', required }` 或 `{ status: 'unknown' }`。鍵盤把每一種對應成各自不同的
本地化訊息 + emoji，並在 `granted` 時觸發 `onUnlocked(feature)`。任何有效兌換都不會顯示通用的「代碼無效」。

代碼集中在一處，`utils/unlocks.ts` 的 `CODE_FEATURES`，是一套**兩階、前置鏈結**的方案。每個系列以一個
**前置代碼**開頭，授予一個*本身不顯示任何東西*的旗標；功能代碼在該前置旗標出現前一律**被拒絕**：
...
`redeemCode` 區分三種結果（granted／prerequisite-missing／unknown）。由 **裝置設定**（裝置作用域兌換，
