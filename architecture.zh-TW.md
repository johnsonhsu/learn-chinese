...
- **正式環境讀取路徑、重用端點。** 擁有者輸入一次管理 secret；**「解鎖」會先向伺服器探測**（以 header 打
  ...
  *這個部署根本沒設定*，端點都回**無法區分的 `403`**，解鎖錯誤訊息會同時點出兩種原因——後者是 **PR PREVIEW
...
> 若想在 preview 上也能操作 triage，請在 Pages 專案的 **Preview** 環境（dashboard）另外加上 `FEEDBACK_ADMIN_SECRET`
> 與 `FEEDBACK_DB`／`FEEDBACK_R2` 兩個 binding。
