...
**Curriculum QA (dev-only, advisory — not a gate).** Beyond the automated gates, `npm run sentence-qa` runs the bank through **≥2 local LLMs** (Ollama) for a grammar/semantics/naturalness review of Taiwan-Traditional usage, and `npm run sentence-qa:report` renders a self-contained HTML viewer that surfaces where the models **disagree**. It's a human review aid — **read-only on the bank, never a gate** (see [`scripts/README.md`](./scripts/README.md)).
