...
```bash
npm test           # 全部
npm run test:unit  # 快速的純邏輯 + 主題單元測試（shared/ + platform/src）
...
- **主題**——解析、premium 解鎖、每個 profile 的覆寫、`body[data-theme]` 套用、備份往返。
- **資料完整性閘門**——出貨的資料庫不含簡體／無法書寫的字形、參照完整、**不含任何個人資料**，且銀行句子用到的每個課程字都能離線書寫。

## 部署

以靜態資源部署到 **Cloudflare Pages**——沒有正式環境（production）伺服器。**部署由 CI 驅動**（`.github/workflows/ci.yml`）：

- **Pull request → preview。** CI 執行測試、以 committed seeds 建置、執行資料完整性閘門，然後部署一個 Cloudflare Pages **preview**（每個 PR 有獨立網址）。
- **合併到 `master` → production。** 相同的建置 + 閘門，指向 production 分支——正式環境只是 preview 流程的鏡像。
- 需要 repo secrets `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID`（Pages:Edit API token）。

本機 build **不再自動部署**——以合併通過測試的 PR 來出貨。可重現建置：工作用的 `platform.db` / `writing-challenge.db` 存放你的本機進度、不進 git；CI 改以清理過、僅含內容的 **`seed/`** 資料庫建置——課程內容變更後以 `npm run seed:dbs` 重新產生。手動緊急通道（部署一個 *preview*）：
