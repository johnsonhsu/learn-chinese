...
- **Reading Chinese**（📖 `reading-chinese`）— 閱讀理解練習：聽句子＋看英文意思，然後從該句子
  自身字元組成的**打散**字塊池中，**依序**點選字來重組整個句子（沒有書寫板）。閱讀被視為與書寫
  **各自獨立的每字技能**——你可以認得一個字卻還不會寫它——因此閱讀熟練度是獨立於書寫熟練度計算的。
- **Reading English**（📗 `reading-english`）— Reading Chinese 的英文對應版本：聽一個銀行句子的
  英文＋看它的中文題目，然後從該句子自身單字組成的**打散**單字塊池中，**依序**點選單字來重組整個
  英文翻譯（沒有鍵盤）。閱讀被視為與 Practice-English 拼字統計**各自獨立的每字技能**——你可以讀懂
  一個還不會拼的單字——它有自己的裝置端儲存，因此兩者絕不互相污染。
...
**soft／節慶**（Sakura、Matcha、Jungle、Christmas）、**retro**（90s、80s Motiv）、**Disney**（Boyish、Girlish），以及
**foil**——兩款付費皮膚 **Gold（暖金箔）** 與 **Silver（冷鉑金）**。一個主題就是 `themes.ts` 註冊表中的
一筆項目；其外觀是一個 `body[data-theme="<id>"]` 區塊——內嵌在 `index.css`，或放在獨立的
`theme/theme-<id>.css` 檔並於 `main.tsx` 匯入。你可以在裝置設定中為 **整台裝置** 設定主題，或在某個 profile 的設定中
**針對該 profile** 設定；有效主題的解析為 `profileOverride ?? device ?? default`。除了兩款 **foil** 皮膚外，其餘主題皆免費；
...
_消費者_。每個 profile 的進度則維持獨立。在匯入時（以及透過離線 scrub，`scripts/bank-fix.py`），
每個句子都會被 **正規化為單一的台灣繁體形式**：簡體 → 繁體，但 台 _和_ 臺 兩者都保留（永不轉換），
...
- **主題解析**——選擇、premium 解鎖、每個 profile 覆寫、`body[data-theme]` 套用皆由 theme-store 測試套件涵蓋（`platform/src/theme/__tests__/theme-store*.test.ts`）。

**課程品質審查（僅供開發、僅供參考——非閘門）。** 除了自動閘門之外，`npm run sentence-qa` 會把句庫送過 **≥2 個本機 LLM**（Ollama），針對台灣繁體的文法／語意／自然度做審查，`npm run sentence-qa:report` 則產生一份自足的 HTML 檢視器，凸顯兩個模型**意見分歧**之處。這是給人類策劃者的輔助工具——**對句庫唯讀、絕非閘門**（見 [`scripts/README.md`](./scripts/README.md)）。
...
`platform/` 有一個小型 Playwright 截圖 harness，用來擷取行銷／建構畫面。
它**不是** app integration test，也**尚未**接到 CI。

- 設定：`platform/playwright.config.ts`
- 截圖 specs：`platform/scripts/marketing-screenshots/marketing.spec.ts`
- 輸出：`platform/public/marketing/`（`landing.png`、`styleguide.png`，以及 `*.meta.json`）
- 執行：`cd platform && npx playwright test --project=desktop`
- 時機：行銷資產需要更新時再手動執行；不連動 deploy，不改動 app code。
...
- **合併到 `master` → production。** 相同的建置 + 閘門。此 Pages 專案為 _direct-upload_，production 分支是 **`learning-chinese`**（不是 `master`），因此 master push 部署 `--branch=learning-chinese`（→ `learnchinese.hsu.mobi`），PR 部署 `--branch=<PR head>`（→ preview）。
...
**工作流程慣例**——開始處理 issue 前，**先拉到最新的 `master`**，再把 feature branch 從這個最新的 `master` 切出來，不要從舊的本地歷史切。

**PR 描述慣例**——**務必在 PR 描述中連結相關 issue**，例如 `Closes #NN` 或 `Related to #NN`，讓這項工作能追溯到它的規格來源。
...
可重現建置：工作用的 `platform.db` / `writing-challenge.db` 存放本機進度、不進 git；CI 改以清理過、僅含內容的 **`seed/`** 資料庫建置。本機 build **不再自動部署**。手動緊急通道（部署一個 _preview_）：
...
| `modules/*`                                 | 七個學習活動（見 [modules/README.zh-TW.md](./modules/README.zh-TW.md)） |
