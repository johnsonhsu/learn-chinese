...
const charSet = [
  ..."我你他她的是在有不沒和很也都要去來看說聽吃喝做喜歡人家朋友學校東西水飯書今天現上下裡們一了這個中大國地到就出齣會也你對",
];
...
for (let i = 0; i < allSentences.length; i++) {
  ...
  console.log(
    `    ${chars.length} chars, ${inSet.length} in set (${pct}%), ${outOfSet.length} out${outDetail}`,
  );
  console.log("");
}
...
console.log(
  `In set: ${allCharsUsed.length - allOut.length} (${Math.round((1 - allOut.length / allCharsUsed.length) * 100)}%)`,
);
