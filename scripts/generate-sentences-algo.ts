...
const WORD_DICT_PATH = join(
  __dirname,
  "..",
  "input-lists",
  "writing-challenge",
  "dictionary_word_2025-12-27.jsonl",
);
const BASE_CHARS_PATH = join(
  __dirname,
  "..",
  "modules",
  "writing-challenge",
...
function categorize(words: DictWord[]): WordBank {
  ...
  for (const [cat, list] of Object.entries(cats)) {
    (bank as Record<string, string[]>)[cat] = list.filter((w) => words.some((dw) => dw.trad === w));
  }
...
const templates: Template[] = [
  // Subject + Verb + Object
  (b) =>
    `${pick(b.pronouns)}${pick(["要", "去", "會", "喜歡"])}${pick(["吃飯", "喝水", "看書", "上學"])}`,
  ...
  (b) =>
    `${pick(b.pronouns)}和${pick(["朋友", "大家", "他們"])}${pick(["一起", ""])}${pick(["去", "吃飯", "看書", "喝水"])}`,
  // Subject + 就 + Verb + 了
  ...
  // Subject + 都 + Verb
  (b) =>
    `${pick(b.pronouns)}都${pick(["要", "會", "喜歡", "在"])}${pick(["吃飯", "看書", "喝水", "這裡"])}`,
  // 這/那 + 個 + Noun + 很 + Adj
  () => `${pick(["這個", "那個"])}${pick(["人", "東西", "地"])}${pick(["很大", "很好", "不對"])}`,
  // 對 + Subject + 來說
  (b) =>
    `對${pick(b.pronouns)}來說${pick(["這個", "學校", "東西"])}${pick(["很好", "很大", "不對"])}`,
  // Longer: Time + Subject + 在 + Location + Verb + Object
  (b) =>
    `${pick(b.timeWords)}${pick(b.pronouns)}在${pick(b.locations)}${pick(["吃飯", "看書", "喝水"])}`,
  // Subject + 不要 + Verb
  (b) => `${pick(b.pronouns)}不要${pick(["去", "吃", "喝", "看", "說", "出去"])}了`,
  // 一 + Verb + 就 + Verb
  (b) =>
    `${pick(b.pronouns)}一${pick(["來", "到", "看", "說", "吃"])}就${pick(["去", "走", "出去", "喜歡"])}了`,
  // Subject + 也 + Verb
...
const charSetStr =
  "我你他她的是在有不沒和很也都要去來看說聽吃喝做喜歡人家朋友學校東西水飯書今天現上下裡們一了這個中大國地到就出齣會對";
...
console.log(
  `Pronouns: ${bank.pronouns.length}, Verbs: ${bank.verbs.length}, Nouns: ${bank.nouns.length}`,
);
...
for (let i = 0; i < sentences.length; i++) {
  ...
  console.log(
    `${String(i + 1).padStart(2)}. ${s}  [${chars.length} chars, ${pct}% in-set${outDetail}]`,
  );
}
...
console.log(`\n--- Summary ---`);
console.log(
  `Total chars: ${allChars.length}, In-set: ${allIn.length} (${Math.round((allIn.length / allChars.length) * 100)}%)`,
);
