...
- **Merged (#9)**: issue + PR templates under `.github/` (bug/feature/content/perf chooser + PR template). The **issue-as-spec workflow** (the `🤖 Task` template + the _Working agreement_ section above) follows here; type/status/priority labels already exist on the repo.
