...
- **Theme** (`platform/src/theme/`): registry in `themes.ts`. `DEFAULT_THEME_ID` = the default _selection_ (currently `indigo`); `ROOT_THEME_ID` = the bare `:root` "Paper" look (`default`) which REMOVES `body[data-theme]`. Apply via `applyThemeToBody`. Resolution: `profileOverride ?? device ?? default`; premium themes gated by a device unlock. The picker groups themes by a `group` field (#129) — default/dark/soft/retro/disney/foil; **14 themes** ship today, of which Gold/Silver are the only two premium (foil).
...
- **Themes.** Default is **Indigo**; **14 themes** ship across grouped picker categories (#129) — default/dark/soft/retro/**Disney**/foil — spanning 80s Motiv (#81), 90s/Retro OS (#91), Jungle, Outer Space, Disney Boyish/Girlish, and seasonal **Christmas** (#128); Gold/Silver remain the only two premium (foil) skins. Christmas is **seasonal** — shown only Nov–Jan, or year-round once code **9980** is redeemed (`theme-christmas-allyear`). The landing menu/dock and the CharTile **status-frame** ring (`.char-tile::after`, the old corner pip is gone) are in.
