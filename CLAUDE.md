...
- **Theme** (`platform/src/theme/`): registry in `themes.ts`. `DEFAULT_THEME_ID` = the default _selection_ (currently `indigo`); `ROOT_THEME_ID` = the bare `:root` "Paper" look (`default`) which REMOVES `body[data-theme]`. Apply via `applyThemeToBody`. Resolution: `profileOverride ?? device ?? default`; premium themes gated by a device unlock. The picker groups themes by a `group` field (#129) — default/dark/soft/retro/disney/foil; **13 themes** ship today, of which Gold/Silver are the only two premium (foil).
...
- **Themes.** Default is **Indigo**; **13 themes** ship across grouped picker categories (#129) — default/dark/soft/retro/**Disney**/foil — spanning 80s Motiv (#81), 90s/Retro OS (#91), Jungle, Outer Space, and Disney Boyish/Girlish; Gold/Silver remain the only two premium (foil) skins. The landing menu/dock and the CharTile **status-frame** ring (`.char-tile::after`, the old corner pip is gone) are in. (Note: `theme-christmas.css` from #128 exists but isn't registered in `themes.ts`/imported in `main.tsx` — an orphaned skin, not a shipped theme.)
...
- **Security & tooling.** Baseline security headers + CSP via Pages `_headers` (#58), a **blocking dependency-audit gate** (prod-runtime, high+, #61), a hardened dev Express server (127.0.0.1 bind + SQL `dbPath` allowlist, #62), a standalone dev-only Sentence Bank admin on its own port (#53), a **`tsc` type-check gate** (#42), and a **blocking lint gate** wired into CI (#85 added it advisory; #86 + scoped sweeps #103–#108 swept the baseline to zero and flipped `continue-on-error` off — `eslint . --max-warnings=0` now fails the build like `tsc`). The _Working agreement_ above (dispatch/merge action-signal labels, the reconcile cue, the artifact sweep, the `investigate` issue type) is now fully implemented.
...
- **In flight:** p1 bug **#97** (`npm run build` only works when uncommitted `input-list/writing-challenge/` files are present) is open; theme spike **#45** is `dispatch`-labeled and queued. The lint pipeline **#86** (+ per-area subtasks #103–#108) is **done** — baseline is zero and CI lint is blocking.
